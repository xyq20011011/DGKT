...
def divide_data():
    ...
    stu_log = {}
    # 打开CSV文件
    with open('anonymized_full_release_competition_dataset.csv', newline='', encoding="ISO-8859-1") as csvfile:
        # 创建一个CSV阅读器对象
    ...
    with open('anonymized_full_release_competition_dataset.csv', newline='', encoding="ISO-8859-1") as csvfile:
        ...
        for row in tqdm(csv_reader):
            stu_id = row[0]
            p_id = problem_code[row[16]]
            response = 0.0 if row[23] == '0' else 1.0
